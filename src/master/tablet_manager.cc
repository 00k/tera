...
void Table::MergeTablets(TabletPtr first_tablet, TabletPtr second_tablet,
    ...
    if (first_tablet->gc_reported_ && second_tablet->gc_reported_) {
        (*merged_tablet)->gc_reported_ = true;
        ++reported_live_tablets_num_;
    }
...
void Table::SplitTablet(TabletPtr splited_tablet,
    ...
    if (splited_tablet->gc_reported_) {
        (*first_tablet)->gc_reported_ = true;
        (*second_tablet)->gc_reported_ = true;
        ++reported_live_tablets_num_; // reported_live_tablets_num_ - 1 + 2
    }
...
void Table::GarbageCollect(const TabletInheritedFileInfo& tablet_inh_info) {
    ...
    while (old_it != tablet->inh_files_.end() && new_it != report_inh_files.end()) {
        if (*old_it == *new_it) {
            ...
            VLOG(10) << "[gc] " << tablet->GetPath() << " release file " << *new_it;
...
void Table::AddInheritedFile(const TabletFile& file) {
    ...
    InheritedFileInfo& inf_file = useful_inh_files_[file.tablet_id][file];
    if (is_gc_disabled && inf_file.ref == 0) {
